...
'''
    ...
    def run_training(self, ollama_model: str, training_file: str, output_model: str,
        ...
        if not os.path.exists(training_file):
            logger.error(f"Archivo no encontrado: {training_file}")
            return False
        ...
        try:
            ...
            script_content = self.create_training_script(
                ollama_model, unsloth_model, f"../{training_file}",
...
def main():
    ...
    try:
        ...
        if not os.path.exists(args.training_file):
            logger.error(f"❌ Archivo no encontrado: {args.training_file}")
            return
        ...
        if success_count > 0:
            ...
            print(f"  🎯 Zero-shot: Mejor generalización, entrenamiento más simple")
            print(f"  🎯 One-shot: Mejor para casos específicos, requiere más datos")
            print(f"  ⚖️ LoRA: Más preciso (16-bit), requiere más memoria")
            print(f"  ⚖️ QLoRA: Más eficiente (4-bit), menor uso de memoria")
            
            print(f"\n📊 BARRAS DE PROGRESO INCLUIDAS:")
            print(f"  ✅ Generación de prompts zero-shot y one-shot")
            print(f"  ✅ Validación de configuraciones de grid search")
            print(f"  ✅ Progreso de experimentos individuales")
            print(f"  ✅ Progreso general de entrenamientos")
            print(f"  ✅ Análisis de distribución de categorías")
        else:
